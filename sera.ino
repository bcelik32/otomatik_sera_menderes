...
int sudegeri = 0;
char receivedData[64]; // Okunan veriyi saklamak için bir karakter dizisi oluşturun
...
if (Serial.available() > 0) {
      ...
      int bytesRead = Serial.readBytesUntil(delimiter, receivedData, sizeof(receivedData) - 1);
if (bytesRead > 0) {
...
if (manuel==1 || manuel==true){
    ...
    if (receivedData[0]== 'q') { // olmazsa tektırnak yerine çift tırnak kullanmayı dene
    catimotor.write(5);
    delay(10);
...
digitalWrite(In3, LOW);
   digitalWrite(In4, LOW);
    ...
    if (receivedData[0]=='g'){
        Serial.println("if calisiyor");
        String orijinalString = receivedData;
int baslangicPozisyon = 1; // Başlangıç pozisyonu (ilk karakter)
String alinanKisim;
 alinanKisim = orijinalString.substring(baslangicPozisyon);
  int servoaci = alinanKisim.toInt();
//lcd.setCursor(0,0);
 // lcd.print("aci: "); //İlk satıra yazalım
 // lcd.println(servoaci);
 motor.write(servoaci);
  delay(100);
      }
